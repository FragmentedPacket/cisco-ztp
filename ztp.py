...
img_cat9k = 'cat9k_iosxe.16.06.04.SPA.bin'
img_cat9k_md5 = 'd3c46a054bb31773e4090899b193c949'

def configure_replace(file, file_system='bootflash:/'):
    config_command = 'configure replace %s%s force' % (file_system, file)
    config_repl = cli(config_command)
    time.sleep(120)

def check_file_exists(file, file_system='bootflash:/'):
...
def deploy_eem_upgrade_script(image):
    install_command = 'install add file bootflash:' + image + ' activate commit'
...
def file_transfer(tftp_server, file, file_system='bootflash:/'):
    ...
    transfer_file = "copy tftp://%s/network-firmware/%s %s vrf Mgmt-vrf" % (tftp_server, file, destination)
...
def upgrade_required():
    ...
    # Check if switch is on approved code: 16.06.04
    match = re.search('Cisco IOS XE Software, Version 16.06.04', sh_version)
...
def verify_dst_image_md5(image, src_md5, file_system='bootflash:/'):
